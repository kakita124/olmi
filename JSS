...
angleRange.addEventListener('change', function() {
    if (angleChanged) {
        ...
        if (netForceValue <= 0 && angle < 90) {
            showNotification('Objeto en Reposo', `El objeto no se mueve porque la fuerza de fricción (${frictionForce.toFixed(2).replace('.', ',')} N) es mayor o igual que la componente gravitatoria paralela (${gravitationalForce.toFixed(2).replace('.', ',')} N). Aumente el ángulo o reduzca la fricción.`, 'warning');
        } else {
...
frictionRange.addEventListener('change', function() {
    if (frictionChanged) {
        ...
        if (netForceValue <= 0) {
            showNotification('Objeto en Reposo', `El objeto no se mueve porque la fuerza de fricción (${frictionForce.toFixed(2).replace('.', ',')} N) es mayor o igual que la componente gravitatoria paralela (${gravitationalForce.toFixed(2).replace('.', ',')} N). Reduzca la fricción o aumente el ángulo.`, 'warning');
        } else {
...
frictionRangePulley.addEventListener('change', function() {
    if (frictionChanged) {
        ...
        if (netForceValue <= 0) {
            showNotification('Objeto en Reposo', `El objeto no se mueve porque la fuerza de fricción (${frictionForce.toFixed(2).replace('.', ',')} N) junto con el contrapeso superan la componente gravitatoria. Reduzca la fricción o ajuste el contrapeso.`, 'warning');
        } else {
            showNotification('Fricción Modificada', `El coeficiente de fricción se ha cambiado a ${parseFloat(this.value).toFixed(2).replace('.', ',')}. Esto afecta la fuerza de fricción que se opone al movimiento.`, 'info');
        }
        frictionChanged = false;
